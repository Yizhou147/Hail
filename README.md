...
1. 在专注页勾选要屏蔽的应用（支持多选、搜索、批量导入），选择或输入倒计时时长，开始专注。
2. 专注开始后，黑名单应用被系统暂停（弹出"专注模式中"提示），无法启动。
3. 雹进入全屏倒计时锁定：覆盖雹的全部页面，拦截返回键与触摸操作，专注期间无法中途退出。
4. 常驻通知持续显示剩余时间（支持通知栏、超级岛等系统呈现方式）。
5. 倒计时结束自动解除：先解除系统暂停，再按开始前的快照恢复各应用原有冻结状态。

- 需要 [Shizuku](https://github.com/RikkaApps/Shizuku) 授权。
- 倒计时支持 1–240 分钟，可保存最多 20 个预设。
