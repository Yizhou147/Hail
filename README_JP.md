...
フォーカスモードは、設定した時間内にブラックリストのアプリをブロックし、気が散るのを防ぎます。

### 使い方

1. フォーカスページでブロックするアプリをチェックします：
   - **検索**：アプリ名またはパッケージ名で検索します。
   - **複数選択**：アプリをまとめてチェックします。
   - **インポート**：右上のインポートボタンをタップします。雹のインストール済みアプリ一覧から手動で選択するか、**クリップボードからインポート**できます。元のパッケージ名リスト形式に対応しています。例：`["io.wallpaperengine.weclient","com.ss.android.ugc.aweme","com.larus.nova","com.example.ourom"]`。単一のパッケージ名にも対応しています。インストール済みでブラックリストにないアプリのみインポートされます。
2. カウントダウン時間（1〜240分、数字の直接入力またはプリセット、最大20個保存）を選択して開始します。
3. 開始後、ブラックリストのアプリはシステムによって一時停止され（「フォーカスモード」という表示が出ます）、起動できなくなります。
4. 雹は全画面カウントダウンロックに入ります：雹のすべてのページを覆い、戻るキーとタッチ操作をブロックするため、途中で終了できません。
5. 常駐通知が残り時間を表示し続けます。通常の通知またはHyperOSアイランドとして表示できます。
6. カウントダウン終了時に自動的に解除されます：まず一時停止を解除し、開始前に取得したスナップショットに従って各アプリの元の凍結状態を復元します。

### 注意

- [Shizuku](https://github.com/RikkaApps/Shizuku) の承認が必要です。
- セッション中にデバイスが再起動した場合、雹は起動後にカウントダウンとロック状態を自動的に復元します。

### HyperOS アイランド

- デフォルトではフォーカス通知がHyperOSアイランドとして表示されます。設定で無効にして通常の通知にすることもできます。
- アイランドの表示にはシステムがフォーカス通知を許可する必要があります。表示されない場合は、HyperCeilerを使用して雹をフォーカス通知のホワイトリストから削除してください。
- 一部の機種では「スマートバッテリー」ポリシーの下で雹の最初の通知が約10秒遅れる場合があります。雹のバッテリーポリシーを「無制限」に設定すると即座に表示されます（雹はバックグラウンドで動作しないため、余分なバッテリー消費はありません）。
